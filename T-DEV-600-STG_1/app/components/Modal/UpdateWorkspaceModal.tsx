...
export const UpdateWorkspaceModal: React.FC<UpdateModalProps> = (
  props: UpdateModalProps
) => {
  const { initialName, workspaceId, visible, onRefresh, onCancel } = props;
  ...
  const updateWorkspace = (id: string, workspace: Workspace) => {
    Trello.updateWorkspace(id, workspace).then((res) => {
      onRefresh();
    });
  };

  const onSubmit = () => {
    updateWorkspace(workspaceId, workspaceInput);
    onCancel();
  };
