...
if ($action === 'add') {
...
} elseif ($action === 'loadLatestMessages') {
  ...
  ORDER BY `created_at` DESC");
    ...
    $pdo = pdo();
    $statement = $pdo->prepare('INSERT INTO `comments` (`content`, `author`, `created_at`) VALUES (:content, :author, NOW())');
    $statement->execute(['content' => $content, 'author' => $author]);
    $statement->closeCursor();
    echo json_encode(['status' => 'ok']);
} elseif ($action === 'loadLatestComments') {
    $lastCommentId = $data['lastId'] ?? null;
    if ($lastCommentId !== '' && $lastCommentId !== null) {
        $pdo = pdo();
        $statement = $pdo->prepare('SELECT * FROM `comments` WHERE `comments`.`id` > :comment_id ORDER BY `created_at` DESC');
        $statement->execute(['comment_id' => $lastCommentId]);
        $comments = $statement->fetchAll(PDO::FETCH_ASSOC);
        $statement->closeCursor();
        echo json_encode(['status' => 'ok', 'comments' => array_map(function ($comment) {
			return message($comment['id'], false, $comment['author'], $comment['content'], $comment['created_at']);
		}, $comments)]);
    } else {
        echo json_encode(['status' => 'error']);
    }
