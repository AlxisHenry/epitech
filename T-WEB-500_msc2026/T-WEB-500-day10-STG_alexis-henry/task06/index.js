...
document.addEventListener("DOMContentLoaded", () => {
    ...
    chats.forEach((chat) => {
        ...
        button.addEventListener("click", (e) => {
            ...
            if (content.length > 0) {
                const author =
                    chat.querySelector(".top .author span").innerHTML;
                fetch("ajax.php", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    setInterval(() => {
        chats.forEach((chat) => {
            ...
            let lastId =
                bubbles.length === 0
                    ? 0
                    : bubbles[bubbles.length - 1].dataset.id || 0;
    ...
    document
        .querySelector("#comments-form button")
        .addEventListener("click", (e) => {
            ...
            let message = document.querySelector(
                '#comments-form textarea[name="message"]'
            ).value;
    ...
    setInterval(() => {
        let comments = document.querySelectorAll(".comments .messages .bubble");
        let lastId =
            comments.length === 0
                ? 0
                : comments[comments.length - 1].dataset.id || 0;
        loadLatestComments(lastId);
    }, DELAY_BETWEEN_LOADS);
});

function addMessageToChat(chat, message) {
    chat.querySelector(".messages").insertAdjacentHTML("beforeend", message);
    chat.querySelector(".messages").scrollTop =
        chat.querySelector(".messages").scrollHeight;
}
...
function loadLatestComments(lastId) {
    console.log(lastId);
    fetch("ajax.php", {
        method: "POST",
        headers: {
        ...
        .then((data) => {
            if (data.status === "ok") {
                data.comments.forEach((comment) => {
                    document
                        .querySelector(".comments .messages")
                        .insertAdjacentHTML("beforeend", comment);
                });
                document.querySelector(".comments .messages").scrollTop =
                    document.querySelector(".comments .messages").scrollHeight;
            }
        });
}
